...
def MainTMAP():
    ...
    formsGraph.remove_node("GP2StringDT")
    ...
    formsGraph.add_edge("GP2StringB", "GP2", color ='#006C84')
    formsGraph.add_edge("GP2", "GP2StringB", color ='#006C84')

    #polarPos = polar(pos["GP2String"][0],pos["GP2String"][1])
    GP2Pos = polarToCartesian(polarPos[0]+srd, polarPos[1]+180)
    pos["GP2"] = GP2Pos

    for p in pos:
        tempPos = pos[p]
        tempPoral = polar(*tempPos)
        pos[p] = polarToCartesian(tempPoral[0],tempPoral[1]+rotation)
        print()

    mapping = {"Emini": "Essence", "ASTpy": "AST", "GP2String": "GP2\nString", "GP2Graph": "GP2\nGraph", "GP2StringB":"GP2\nString\nB"}
    ...
    figure, axes = plt.subplots(figsize=(11.2,12))
    ...
    plt.tight_layout()
    plt.savefig("CP_2024map.svg",bbox_inches='tight')
    plt.show()
